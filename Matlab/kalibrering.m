...
% len = zeros([1 length(name)]);
% for i = length(name)
%     len(i) = length(data.(name{i}).processor_millis);
% end

soon_meanvalue_Co2 = struct; % skapar array att fylla värdena i, för att sedan dela och få medelvärde
...
for i = 1:length(name)
    ...
    CozIr_struct.(name{i}) = movmean(data.(name{i}).CozIr_Co2_filtered, movmean_by_amount);
...
for i = 1:length(name)
        ...
        soon_meanvalue_Co2.(name{i}) =data.(name{i}).CozIr_Co2_filtered; % adderar j:te komponentens data från enhet i, för alla i och j

end

AA = struct2cell(soon_meanvalue_Co2);
sum_mean_Co2 = plus(AA{:});
meanvalue_Co2 = sum_mean_Co2./length(name);
...
%k = [0.9977, 0.9683, 0.9872, 1.153, 1.025, 1.002, 0.8817, 0.9849];

%m = [-11.45, -68.97, -58.6, 286.8, -11.75, -37.22, 112.1 , 13.22];
...
for i = 1:length(name)
    ...
    for j = 1:length(data.(name{i}).GPS_hour)
        ...
        CozIr_array_filtered_corrected(i,j) = k(i).*data.(name{i}).CozIr_Co2_filtered(j) + m(i);
